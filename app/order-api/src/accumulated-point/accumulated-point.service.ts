...
export class AccumulatedPointService {
  ...
  async getPointsHistory(
    ...
    // Get accumulated point of user
    const accumulatedPoint = await this.accumulatedPointRepository.findOne({
  ...
  async getTotalPointsByUserSlug(
    ...
    let accumulatedPoint = await this.accumulatedPointRepository.findOne({
      where: { user: { id: user.id } },
    });
