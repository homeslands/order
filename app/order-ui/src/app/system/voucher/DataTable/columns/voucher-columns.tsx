  ...
  return [
    {
      ...
      header: ({ table }) => (
        <Checkbox
          checked={selectedVouchers.length === table.getRowModel().rows.length}
      ...
      cell: ({ row }) => {
        ...
        return (
          <Checkbox
            checked={selectedVouchers.some(v => v.slug === voucher.slug)}
            onCheckedChange={(value) => {
              row.toggleSelected(!!value)
              updateSelectedVouchers(
                value ? [...selectedVouchers, voucher] : selectedVouchers.filter((v) => v.slug !== voucher.slug)
              )
            }}
