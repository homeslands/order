...
export function useSystemPointTransactions({
  pageSize = 10,
}: UsePointTransactionsProps) {
  const today = moment()
  const [filters, setFilters] = useState<UsePointTransactionsFilters>({
  ...
  const {
    ...
    queryKey: ['pointTransactions', filters],
    queryFn: async ({ pageParam }) => {
      const params: IPointTransactionQuery = {
        page: pageParam as number,
        size: pageSize,
  ...
  const exportAll = useCallback(async () => {
    ...
    setIsExportingAll(true)
    try {
      const blob = await exportAllSystemPointTransactions(
        filters.fromDate,
        filters.toDate,
        filters.type == PointTransactionType.ALL ? undefined : filters.type,
      )
      const filename = `point-transactions-${new Date().toISOString().split('T')[0]}.pdf`
      saveAs(blob, filename)
    } finally {
      setIsExportingAll(false)
    }
